...
def construct_graph(features, metric_type = 'static_KNN', knn_size = None, epsilon = None):
    if metric_type == 'static_KNN':
        if not knn_size is None:
            print('[ Using KNN-graph as input graph: {} ]'.format(knn_size))
            adj = kneighbors_graph(features, knn_size, metric='cosine', include_self=True)
...
def calculate_index(labels, predicts):
    ...
    predicts = np.argmax(predicts.cpu().detach().numpy(), axis=1)
    labels = labels.cpu()
...
def load_datamat():
    ...
    train_rate = 0.4
    val_rate = 0.2
    idx_train = range(int(sample_num * train_rate))
    idx_val = range(int(sample_num * train_rate), int(sample_num * train_rate + sample_num * val_rate))
    idx_test = range(int(sample_num * train_rate + sample_num * val_rate), int(sample_num))
